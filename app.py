...
header = dbc.Row(
    [dbc.Col([html.H1("Ship-to-Ship transfer detection", id="header", className="mb-4")])]
)
        ...
        dbc.Col(
            [
                ...
                dcc.Dropdown(
                    ...
                    value=list_of_unique_dates[85]
                    if list_of_unique_dates
                    else None,  # Sets the default value to the first date
                ),
                html.Br(),
...
interactive_map = dbc.Row(
    [
        dbc.Col(
            [
                dcc.Graph(id="base-map", config={"displayModeBar": False}, clickData=None),
                html.Br(),
            ]
        )
    ]
        ...
        dbc.Row(
            [
                dbc.Col(
                    sidebar,
                    width=2,
                    style={"textAlign": "left", "padding": "3rem 1rem"},
                ),
                dbc.Col(
                    [
                        control,
...
def update_end_date_options_and_frame_date(start_date):
    if start_date:
        ...
        end_dates = [
            {"label": date, "value": date} for date in list_of_unique_dates if date > start_date
        ]
...
def update_summary_table(n_clicks, data, start_date, end_date):
    ...
    if n_clicks and data:
        ...
        stats = [
            ...
            {
                "description": "Revisit frequency (days)",
                "statistics": round(revisit_frequency, 2),
            },
        ]
...
@app.callback(
    ...
    [State("end-date", "value"), State("frame-date", "children")],
)
...
def update_map(frame_date, data):
    ...
    if data:
        ...
        fig = px.scatter_map(
            ...
            #      },
            color_discrete_map={"STS": "red", "Ship": "green", "AIS": "yellow"},
        )
        fig.update_traces(
...
@app.callback(
    ...
    [State("data-table", "data"), State("frame-date", "children")],
)
def display_closest_match_table(clickData, data, frame_date):
    if clickData is None:
        ...
        if prediction == "STS":
            lat, lon = (
                float(df_click.iloc[0]["latitude"]),
                float(df_click.iloc[0]["longitude"]),
            )
            df_ais["distance"] = df_ais.apply(
                lambda row: distance.distance((lat, lon), (row["latitude"], row["longitude"])).km,
                axis=1,
            )
...
@app.callback(Output("click-output-data", "data"), Input("base-map", "clickData"))
...
@app.callback(Output("image-placeholder", "src"), Input("base-map", "clickData"))
