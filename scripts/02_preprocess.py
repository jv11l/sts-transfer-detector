...
from sar_sts_detection.data.preprocessing import (
    apply_mask_to_image,
    create_file_path,
    load_raster_image,
...
def mask():
    ...
    image_file = create_file_path(os.listdir(RAW_IMAGE_DIR)[10], RAW_IMAGE_DIR)
    print(f"Loading raster image {image_file}")
    band, meta, crs = load_raster_image(image_file)
    print(band.shape)
    print(f"Projecting ocean mask to the image CRS: {meta['crs']}")
    gdf_ocean_mask = transform_ocean_mask_to_image_crs(crs, gdf_ocean_mask)
