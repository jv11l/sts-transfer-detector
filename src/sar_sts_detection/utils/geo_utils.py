...
def list_of_ships_and_coords_masked(
    results: object, transform: object, transformer: object, list_of_idx: list
) -> list:
    """
    ...
    for tile_idx, tile_results in enumerate(results):
        for detected_ship in range(len(tile_results)):
            x_center, y_center = get_true_pixel_masked(
                list_of_idx[tile_idx], tile_results, detected_ship
            )
